...
Expression (\ref{criticalPointWithP}) predicts $r_{C} = 0.5 \cdot 1 - 0.5 + 1 = 3$ implying that as soon as the synergy factor exceeds 3 cooperators will benefit from a larger payoff than defectors.
\autoref{fig:secondProof} confirms the shift of the critical point from 5.0 (classical public goods game, $i=1$) and displays a probability to cooperate of 0.5 for a synergy factor of 3.
